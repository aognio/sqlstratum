...
!!! note "Release context"
    `0.4.0` is the release that substantially expands the query DSL through portable predicates,
    set operations, explicit ordering helpers, and stronger MySQL runtime validation. For a prose
    walkthrough with examples, see [Latest release](latest-release.md).
...
Compile-time capability is only part of the story. On the runtime side, `0.4.0` also broadens
execution-level MySQL validation through opt-in real-server integration tests for:

- predicate execution
- set-query execution
- `fetch_one`
- `scalar`
- sync and async runner paths
