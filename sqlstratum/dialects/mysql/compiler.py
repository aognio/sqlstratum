"""MySQL compiler for sqlstratum AST (compile-only MVP)."""
...
class _Compiler:
    ...
    def _compile_select(self, query: ast.SelectQuery) -> str:
        if query.offset is not None and query.limit is None:
            raise UnsupportedDialectFeatureError(
                "mysql",
                "OFFSET without LIMIT",
                hint="Use LIMIT(n).OFFSET(m) when compiling for MySQL.",
            )
        ...
        for join in query.joins:
            if join.kind == "INNER":
                ...
                raise UnsupportedDialectFeatureError(
                    "mysql",
                    "FULL OUTER JOIN",
                    hint="MySQL does not support FULL OUTER JOIN directly.",
                )
            else:
    ...
    def _compile_set(self, query: ast.SetQuery) -> str:
        ...
        if query.order_by:
            parts.append("ORDER BY " + ", ".join(self._compile_set_order(o) for o in query.order_by))
        if query.limit is not None:
            parts.append("LIMIT " + self._bind(query.limit))
        if query.offset is not None:
            parts.append("OFFSET " + self._bind(query.offset))
    ...
    def _compile_set_order(self, order: OrderSpec) -> str:
        expr = order.expr
        if isinstance(expr, Column):
            return f"{self._quote(expr.name)} {order.direction}"
        if isinstance(expr, AliasExpr):
            return f"{self._quote(expr.alias)} {order.direction}"
        return self._compile_order(order)
